...
def generate_prompts(content_brief, network_type, constraint):
    # prompt = '''
    ...
    #         '''.format(network_type, content_brief, constraint)
    prompt = '''
...
def generate_post_text(input_prompt, network_type, post_type):
    ...
    completion = openai.ChatCompletion.create(model=model_id,
                                              messages=[{"role": "user", "content": prompt_for_test_generation}])
